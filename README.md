...
#KEY FEATURES
	-WiFi Access Point or Client mode, for remote administration via Web browser.
	-Dual mode Signal Detection, by RSSI level or by Logic Signal from Carrier detection or CTCSS/DCS (external) detection.
	-Comprehensive hyseteresis on both voltage domain (only on RSSI Mode) and time domain (on both RSSI and Logic Mode) ensure reliable repeater functionality even in high noise environment.
	-“Calm-Down”.
	-Courtesy Tone, Tail End Tone and CW Beacon, each with it's own detailed setup.
	-User defined Time out and Repeater Tail intervals.
	-2x16 LCD Display for basic head-up display.
	-Manual Beacon sending by Web Browser button or hardware connected button.
    -Anti-Kerchunking and Continuous interfering signal protection.
	-Back-up for user defined settings.
	-Different audio levels can be set for each of audio tones generated by the controller.
	-Serial output for debugging and monitoring various informations, selectable from Web page.
...
When a a signal is received and the repeater start transmitting, a Time Out Timer (Repeater TOT) start.
